...
/*  Serial.println(ADMUX);
  ...
  */
  ...
  for(x=0; x<8; x++) {
    ...
    if(level < 0L)      c = 0;
    else if(level > NUMPIXELS) c = NUMPIXELS; // Allow dot to go a couple pixels off top
  ...
  // XXX matrix.writeDisplay();
  // XXX Cheat here and take the max of the peaks
  //max /= (255 / NUMPIXELS);
  for (int i = 0; i < NUMPIXELS; i++) {
    if (i < max) {
      pixels.setPixelColor(i, 32, 32, i/8);
    }
    else {
